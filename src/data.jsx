    ...
    knowhow: [
      {
        ...
        tips: [
          {
            title: '构图',
            tip: '低机位 + 长焦压缩空间感，主角占据黄金分割点',
            prompt: `[镜头] 低机位 50mm 长焦，机位高度约 30cm，镜头轻微仰角 8°；[构图] 主角位于画面右侧 2/3 黄金分割点，前景留出虚化空间；[背景] ${item.tags[0]}氛围环境柔焦处理，景深 f/1.8；[运动] 镜头极缓慢推进（0.5s 推进 30px），让观众自然锁定主角。`,
          },
          {
            title: '光影',
            tip: '顶光勾轮廓 + 底部冷色补光，避免面部过曝',
            prompt: `[主光] 顶光 45° 角，色温 5600K，功率约 70%，仅勾勒发丝与肩部轮廓；[补光] 底部反光板（冷色蓝 7500K，强度 20%）柔化下颌阴影；[氛围光] 背景微弱 ${item.tags[0]}主题点光源（红/蓝/琥珀，强度 15%），营造情绪基调；避免面部超过 1.0 stop 过曝。`,
          },
        ],
      },
      {
        ...
        tips: [
          {
            title: '节奏',
            tip: `前 3 秒必须抓住注意力，${item.duration} 内完成情绪起承转合`,
            prompt: `[节拍] 采用 3-2-1 法则 — 前 3 秒信息密度爆发（至少 2 个画面元素变化），接 2 秒呼吸缓冲，1 秒情绪钉子；[剪辑] 硬切 + 匹配剪辑（match cut），让动作在两个镜头间延续；[音效] 在情绪转折点插入 0.3s 静音 + 一声低频 drop，制造"耳朵记忆点"；总时长 ${item.duration}，保证前 30% 时长完成钩子抓取。`,
          },
          {
            title: '运镜',
            tip: '以人物为锚点，机位随情绪走',
            prompt: `[运镜曲线] 情绪递增段使用平移推进（push-in），速度 1px/帧；情绪爆发段切为环绕镜头（orbit），180° 弧线，2.5s 完成；回落段切回固定机位让观众情绪沉淀；所有运镜保留 6% 头部空间，不得让主角顶到画框。`,
          },
        ],
      },
      {
        ...
        tips: [
          {
            title: '配色',
            tip: `主调 2-3 种，参考${item.tags[0]}常用色板`,
            prompt: `[主色] 选定 2 种 ${item.tags[0]}题材常用色（建议暖 + 冷的互补组合，如琥珀橙 #C47A3E + 深湖蓝 #2B4A6B）；[辅色] 1 种低饱和中性色做过渡（奶油白 #F3EDE0 或石墨灰 #4A443C）；[调色曲线] 高光偏暖、阴影偏冷，色相环距离控制在 150-180°；[LUT] 整片套用统一 LUT，强度 60%，保证镜头间一致性。`,
          },
          {
            title: '情绪钉子',
            tip: '最后 1 秒给观众一个可记忆的画面',
            prompt: `[最后一帧] 回到标题视觉或主角的标志性动作，构图回归中心对称，让画面"定格"；[字幕动画] 若有标题字幕，用 easeOutBack 从 0.94 scale 弹入，停留 0.6s 后淡出；[留白] 结束前 0.3s 画面亮度降至 60%，给观众"余韵时间"；整个收束段不要新增信息量，让前面的情绪落地。`,
          },
        ],
      },
    ],
